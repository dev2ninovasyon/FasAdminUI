...
import { getDovizKurlariOtuzAralik } from "@/api/VeriIslemleri/DovizKurlari";
...
const DovizKurlariOtuzAralik: React.FC<Props> = ({ verileriCekTiklandimi }) => {
  ...
  const fetchData = async () => {
    try {
      const dovizKurlariOtuzAralik = await getDovizKurlariOtuzAralik(
        user.token || ""
      );

      const rowsAll: any = [];
      dovizKurlariOtuzAralik.forEach((veri: any) => {
        const newRow: any = [
...
export default DovizKurlariOtuzAralik;
